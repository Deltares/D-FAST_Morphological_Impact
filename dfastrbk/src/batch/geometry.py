...
def find_distances_to_points(line_coords: np.ndarray, new_points: np.ndarray):
    ...
    cumulative_distances = calculate_curve_distance(line_coords[:,0],
                                                   line_coords[:,1])
    ...
    segment_distances = np.linalg.norm(new_points[:, None] - line_coords[:-1],
                                       axis=2)

    # 3) Add on the “already‐traveled” distance along the curve
    valid_distances = np.where(mask,
                               cumulative_distances[:-1] + segment_distances,
                               np.inf)
...
def read_xyc(
    filepath: Path, num_columns: int = 2
...
def project_km_on_line(
    line_xy: np.ndarray, xykm_np: np.ndarray
) -> np.ndarray:
    """
