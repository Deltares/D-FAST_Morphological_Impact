...
class Test_XykmData_initialize_data:
    def test_keep_all_nodes_and_faces(self):
        ...
        with (
            patch(
                "dfastmi.batch.DflowfmReporters.XykmDataReporter", autospec=True
            ) as xykm_data_reporter,
            patch(
                "dfastmi.batch.XykmData.filter_faces_by_node_condition"
            ) as filter_faces_by_node_condition,
        ):
            xykm_data = XykmData(xykm_data_reporter)
            filter_faces_by_node_condition.return_value = (
                xni,
                yni,
            ...
            numpy.testing.assert_array_equal(
                xykm_data.face_node_connectivity_index, face_node_connectivity_index
            )
    ...
    def test_determine_xykm_data(self):
        ...
        with (
            ...
            patch(
                "dfastmi.batch.XykmData.project_xy_point_onto_line"
            ) as project_xy_point_onto_line,
