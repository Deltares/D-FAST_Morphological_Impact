...
%\cleardoublepage
\title{D-FAST\\ Morphological Impact 3.0}
\subtitle{}
...
\begin{tabular}{ll}
Description: & \dfastmi version 3.0.0 \\
Version: & 3.0.0 \\
Status & release
\end{tabular}
...
This new version implements a generalized algorithm for estimating the sedimentation length and the long-term morphological response of the main channel to changes, so-called measures, in the flood plains.
The tool requires the results of pairs of \dflowfm computations (with and without the measure) for a given set of flow conditions.
}}
