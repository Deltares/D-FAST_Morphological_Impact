...
class CelerDischarge(ICelerObject):
    ...
    def validate(self) -> "CelerDischarge":
        ...
        if self.cdisch == (0.0, 0.0):
            raise ValueError(
                f'The parameter "CelerQ" must be specified for branch "{branch_name}", '
    ...
    def get_celerity(self, discharges: Vector) -> Vector:
        ...
        celerity = tuple(
            self.cdisch[0] * pow(discharge, self.cdisch[1]) for discharge in discharges
        )
        return celerity
...
class CelerProperties(ICelerObject):
    ...
    def validate(self) -> "CelerProperties":
        ...
        if prop_q_length != prop_c_length:
            raise LookupError(
                f'Length of "PropQ" and "PropC" for branch "{branch_name}", '
        ...
        if prop_q_length == 0:
            raise ValueError(
                f'The parameters "PropQ" and "PropC" must be specified for '
    ...
    def get_celerity(self, discharges: Vector) -> Vector:
        ...
        celerity = tuple(
            self._get_celerity(discharge, self.prop_q, self.prop_c)
