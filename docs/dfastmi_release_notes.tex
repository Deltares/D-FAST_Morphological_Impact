...
\section{Version \dfmiversion}

\subsection{Key changes summary}
Release validated for all Rhine and Meuse branches including the Grensmaas.
The key changes of this update are:

\begin{itemize}
\item New river configuration file containing an extended list of required discharge conditions for the Grensmaas, and updated celerities for all Rhine/Meuse branches and reaches. (DFAST-321)
\end{itemize}
...
An overview of all closed issues since the release of version 3.1.1 is given in the table below.

\begin{longtable}{l|l|p{8cm}}
\caption{List of resolved issues for version \dfmiversion} \\
Issue & Type & Description \\ \hline
\endfirsthead
\multicolumn{3}{l}{\textsl{(continued from previous page)}} \\
Issue & Type & Description \\ \hline
...
\endfoot
\endlastfoot
DFAST-321 & Feature & Updated rivers configuration: new discharge levels for Grensmaas, and new celerities for all branches (added in 3.3.0) \\
DFAST-322 & Bug & The discharge levels weren't updated unless the user updated the threshold discharge (fixed in 3.3.0) \\
\end{longtable}

\section{Version 3.2.0}

\subsection{Key changes summary}
Release validated for Rhine and Meuse applications (except for the Grensmaas section).
The key changes of this update are:

\begin{itemize}
\item The impacted length is now the computed as the duration of the discharge period times the associated bed celerity \emph{summed over all discharge levels at which the barriers are opened} ($Q > Q_\text{stagnant}$).
Previously, the contribution of the period during which the intervention was not active ($Q < Q_\text{thr}$) was ignored.
Hence, the computed impacted length is now larger than before if a threshold discharge $Q_\text{thr} > Q_\text{stagnant}$ is specified.
This change breaks backward compatibility with previous \dfmi versions and WAQMORF; previous versions of \dfmi and WAQMORF should no longer be used. (DFAST-313)
\item Support for typical \dflowfm Fourier files created by \dflowfm using the default settings as specified in the \file{fourier\_last\_s.fou} file. (DFAST-137)
\end{itemize}
...
\endfoot
\endlastfoot
DFAST-317 & Doc & Add Meuse validation cases to \dfmi validation document (added in 3.2.0) \\
DFAST-313 & Bug & Morphological effective period should depend on the stagnant discharge instead of threshold discharge (fixed in 3.2.0) \\
DFAST-299 & Bug & \dfmi results file contained invalid \texttt{face\_node\_connectivity} data when using Fourier files as data source (fixed in 3.2.0) \\
DFAST-298 & Bug & Classic WAQMORF mode failed for \dflowfm NetCDF simulation files when low flow condition was not needed (fixed in 3.2.0) \\
DFAST-213 & Bug & The threshold discharge in the GUI was not always set to the value specified in the loaded config-file (fixed in 3.2.0) \\
DFAST-279 & Bug & \dfmi corrects for differences in mesh topology between reference and intervention runs (fixed in 3.1.2) \\
\end{longtable}

\section{Version 3.1.1}

\subsection{Key changes summary}
Release validated for use within the Rhine branches.
...
\begin{itemize}
\item Not yet validated for Meuse applications.
\end{itemize}
...
\endfoot
\endlastfoot 
DFAST-241 & Doc & Add \dfmi validation document (added in 3.1.1) \\
DFAST-242 & Bug & Revolve vulnerabilities in GDAL and libjpeg (fixed in 3.1.1) \\
DFAST-137 & Feature & Support for \dflowfm Fourier-files (added in 3.1.0) \\
\end{longtable}
