\documentclass[git]{deltares_manual}

\usepackage{tikz}
\usepackage[pipeTables=true]{markdown}
\usepackage{verbatimbox}

%------------------------------------------------------------------------------
\newcommand{\dfastmi}{\textrm{D-FAST~Morphological~Impact}\xspace}
\newcommand{\dfastbe}{\textrm{D-FAST~Bank~Erosion}\xspace}
\newcommand{\dfmi}{\textrm{D-FAST~MI}\xspace}
\newcommand{\dflowfm}{\textrm{D-Flow~FM}\xspace}
\DeclareSIUnit\year{\text{yr}}

\hypersetup
{
    pdfauthor   = {Deltares},
    pdftitle    = {\dfastmi},
    pdfkeywords = {Deltares User Manual \dfmi}
}

%------------------------------------------------------------------------------
%

%\makeindex

%------------------------------------------------------------------------------
%
\begin{document}
\pagestyle{empty}
\includepdf[pages=1, offset=72 -70]{cover/D-FAST-omslag-D-FAST Morphological Impact-UM.pdf} % links-rechts past precies
\cleardoublepage
\title{D-FAST\\ Morphological Impact}
\subtitle{}
\manualtype{User Manual}
\distribution{Released for: \newline \phantom{M} \dfmi version 3.2.0}
\version{\the\year.\the\month}

\author{ }

\setgitdirectory{../.git}
\deltarestitle
%
\include{chapters/intro}
\include{chapters/get_started}
\include{chapters/guidance}
\include{chapters/conceptual_description}
\include{chapters/application}
\include{chapters/example}

%\textit{}\include{chapters_usermanual/guide}

\nonumchapter{References}
\bibliography{dfastmi}

\appendix
\include{chapters/file_formats}
\include{chapters/sim2ugrid}

\chapter{Background information}\label{Sec:memo_Sieben24}
The following pages reproduce a memo by Arjan Sieben on the choice of the representative flow conditions and bed celerities for the Rhine branches and the Meuse river.

%The next line should work, but doesn't seem to work well together with the Deltares manual class
%\includepdf[pages=-, offset=72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden.pdf}
\includepdf[pages=1, offset=-72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}
\includepdf[pages=2, offset=72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}
\includepdf[pages=3, offset=-72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}
\includepdf[pages=4, offset=72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}
\includepdf[pages=5, offset=-72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}
\includepdf[pages=6, offset=72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}
\includepdf[pages=7, offset=-72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}
\includepdf[pages=8, offset=72 -70, frame=true, scale=0.9]{figures/afvoerblokken plus uitbreiding voortplantingssnelheden upd (002).pdf}

\pagestyle{empty}
\cleardoublepage
\mbox{}
\includepdf[pages=2, offset=-72 -70]{cover/D-FAST-omslag-D-FAST Morphological Impact-UM.pdf} % links-rechts past precies
\end{document}
