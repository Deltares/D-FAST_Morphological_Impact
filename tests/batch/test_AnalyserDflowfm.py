...
class Test_AnalyserDflowfm:
    ...
    def _set_file_name_based_on_discharge(self):
        self.filenames[self.initialized_config.discharges[0]] = (
            "file1.extension",
            "file1.extension2",
        )
        self.filenames[self.initialized_config.discharges[1]] = (
            "file2.extension",
            "file2.extension2",
        )
        self.filenames[self.initialized_config.discharges[2]] = (
            "file3.extension",
            "file3.extension2",
        )
    ...
    def test_analyser_early_return_when_filenames_missing(
        self,
        tmp_path,
        ...
        report_data = analyser.analyse(
            self.nwidth, self.filenames, self.xykm, self.plotops
        )

        assert analyser.missing_data
        assert report_data == None

    def test_analyser_early_return_when_xykm_missing(
    ...
    def test_analyse_without_xykm_and_with_old_zmin_zmax(
        self, tmp_path, display: bool, setup
    ):
        ...
        with (
            ...
            analyser = AnalyserDflowfm(
                display, self.report, old_zmin_zmax, outputdir, self.initialized_config
            )
            report_data = analyser.analyse(
                self.nwidth, self.filenames, self.xykm, self.plotops
            )
    ...
    @pytest.mark.parametrize("display", [True, False])
    def test_analyse_with_xykm_and_with_old_zmin_zmax(
        self, tmp_path, display: bool, setup
    ):
        self.initialized_config.needs_tide = False
        old_zmin_zmax = True
    ...
    def test_analyse_with_xykm(self, tmp_path, display: bool, setup):
        ...
        with (
            ...
            patch(
                ...
                return_value=sedimentation_data,
            ),
        ):
