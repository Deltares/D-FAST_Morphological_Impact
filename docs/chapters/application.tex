...
Verify that it's appropriate to use \dfastmi instrument; see \autoref{Chp:Guidance}.
Determine the branch and reach on which it's located; use the \dfmi GUI for this\footnote{The reaches are specified in the GUI both by a descriptive name and an approximate indication of the river chainage.
...
\item Determine, given the branch/reach on which the intervention is location, which \dflowfm simulations should be carried out.
\footnote{Note that in WAQMORF and version 2 of \dfastmi, the discharges depended on the characteristics of the intervention.
...
\item Run \dfastmi to compute the per grid point in the main channel, the following three variables
