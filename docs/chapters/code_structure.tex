...
\dfastmi Python module is subdivided into 7 files:

\begin{itemize}
\item \keyw{\_\_main\_\_.py} module level file containing the version number
\item \keyw{cmd.py} main file to start the module in any mode
...
\subsection{main function and command line interface \keyw{cmd.py}}
...
\begin{itemize}
\item \keyw{parse\_arguments} parses the command line arguments and decide on batch mode or legacy interactive mode
\end{itemize}
...
The \keyw{io.py} file contains all generic file handling routines for reading configuration files, processing netCDF input and output files, and functions to support legacy input and output formats.
