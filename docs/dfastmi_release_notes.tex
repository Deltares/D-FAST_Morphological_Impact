...
\begin{itemize}
\item The impacted length depends now always on the discharge at which the barriers are opened and no longer on the discharge at which the intervention becomes active.
This change breaks backward compatibility with previous \dfmi versions and WAQMORF; older versions should no longer be used.
...
\endfoot
\endlastfoot
DFAST-313 & Bug & Morphological effective period should depend on the stagnant discharge instead of threshold discharge (fixed in 3.1.3) \\
DFAST-299 & Bug & \dfmi results file contained invalid \texttt{face\_node\_connectivity} data when using Fourier files as data source (fixed in 3.1.3) \\
DFAST-298 & Bug & Classic WAQMORF mode failed for \dflowfm NetCDF simulation files when low flow condition was not needed (fixed in 3.1.3) \\
DFAST-213 & Bug & The threshold discharge in the GUI was not always set to the value specified in the loaded config-file (fixed in 3.1.3) \\
