...
def analyse_and_report_dflowfm(
            ...
            if not missing_data and not Q[i] is None:
                ...
                if rsigma[i] == 1 or Q[i] <= q_threshold:
                    ...
                    if needs_tide:
                        ...
                        ApplicationSettingsHelper.log_text(
                            "no_file_specified_q_only", dict={"q": q}, file=report
                        )
    ...
    if 0 in filenames.keys():  # the keys are 0,1,2
        for i in range(3):
            if not missing_data and not Q[i] is None:
                dzq[i] = get_values_fm(
                    i + 1, Q[i], ucrit, report, filenames[i], n_fields, dxi, dyi, iface
                )
                if dzq[i] is None:
                    missing_data = True
            else:
    ...
    if not missing_data:
        ...
        if plotops["plotting"]:
            if FNCi.mask.shape == ():
                ...
                nnodes = (
                    numpy.ones(FNCi.data.shape[0], dtype=numpy.int64)
                    * FNCi.data.shape[1]
                )
            else:
            ...
            if plotops["saveplot"]:
                ...
                if plotops["saveplot_zoomed"]:
                    dfastmi.plotting.zoom_xy_and_save(
                        fig,
                        ax,
                        figbase,
                        plotops["plot_ext"],
        ...
        if xykm is not None:
            ...
            if display:
                if sedvol.shape[1] > 0:
                    ...
                    print(
                        "Max                         : {:13.6f} m3 {:13.6f} m3 {:13.6f} m3".format(
                            sedvol[0, :].max(), sedvol[1, :].max(), sedvol[2, :].max()
                        )
                    )
                    print(
                ...
                if erovol.shape[1] > 0:
                    ...
                    print(
                        "Max                         : {:13.6f} m3 {:13.6f} m3 {:13.6f} m3".format(
                            erovol[0, :].max(), erovol[1, :].max(), erovol[2, :].max()
                        )
                    )
                    print(
                        "Total   ({:15.3f} m2): {:13.6f} m3 {:13.6f} m3 {:13.6f} m3".format(
...
def get_values_fm(
    ...
    if filenames[0] == "":
        ApplicationSettingsHelper.log_text(
            "no_file_specified", dict={"q": q}, file=report
        )
        ApplicationSettingsHelper.log_text("end_program", file=report)
        return None
    elif not os.path.isfile(filenames[0]):
        ApplicationSettingsHelper.log_text(
            "file_not_found", dict={"name": filenames[0]}, file=report
        )
        ApplicationSettingsHelper.log_text("end_program", file=report)
        return None
    else:
    ...
    if not os.path.isfile(filenames[1]):
        ApplicationSettingsHelper.log_text(
            "file_not_found", dict={"name": filenames[1]}, file=report
        )
        ApplicationSettingsHelper.log_text("end_program", file=report)
        return None
    else:
    ...
    for ifld in range(n_fields):
        ...
        # reference data
        u0 = GridOperations.read_fm_map(
            filenames[0], "sea_water_x_velocity", ifld=ifld
        )[iface]
        v0 = GridOperations.read_fm_map(
            filenames[0], "sea_water_y_velocity", ifld=ifld
        )[iface]
        umag0 = numpy.sqrt(u0**2 + v0**2)
        h0 = GridOperations.read_fm_map(
            filenames[0], "sea_floor_depth_below_sea_surface", ifld=ifld
        )[iface]

        # data with measure
        u1 = GridOperations.read_fm_map(
            filenames[1], "sea_water_x_velocity", ifld=ifld
        )[iface]
        v1 = GridOperations.read_fm_map(
            filenames[1], "sea_water_y_velocity", ifld=ifld
        )[iface]
        umag1 = numpy.sqrt(u1**2 + v1**2)

        dzq1 = dfastmi.kernel.core.dzq_from_du_and_h(
            umag0, h0, umag1, ucrit, default=0.0
        )
...
def comp_sedimentation_volume(
    ...
    sedarea, sedvol, sed_area_list, wght_area_tot = DetectAndPlot.detect_and_plot_areas(
        dzgemi,
        dzmin,
        EFCi,
        wght_area_tot,
    ...
    eroarea, erovol, ero_area_list, wght_area_tot = DetectAndPlot.detect_and_plot_areas(
        -dzgemi,
        dzmin,
        EFCi,
        wght_area_tot,
...
def xynode_2_area(
    xn: numpy.ndarray, yn: numpy.ndarray, FNC: numpy.ndarray
) -> numpy.ndarray:
    """
...
def filter_faces_by_node_condition(
    xn: numpy.ndarray,
    yn: numpy.ndarray,
    FNC: numpy.ma.masked_array,
    ...
    rxn, ryn, rFNC, iface, inode = filter_faces_by_face_condition(
        xn, yn, FNC, fcondition
    )
    return rxn, ryn, rFNC, iface, inode


def filter_faces_by_face_condition(
    xn: numpy.ndarray,
    yn: numpy.ndarray,
    FNC: numpy.ma.masked_array,
    condition: numpy.ndarray,
