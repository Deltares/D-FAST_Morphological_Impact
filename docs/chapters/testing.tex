...
SonarCloud doesn't provide the code duplication graph anymore, but the duplication in the \dfmi code is still reported to be 0.0\% although there is about 12\% code duplication in the coding of the tests.
The code coverage data is no longer passed to SonarCloud, but we have seen above that the coverage is still about 90\%.
SonarCloud reports a major jump in the code issues in May 2025.
Since the code wasn't changed during that period, it must be caused by an update to the SonarCloud analysis.
