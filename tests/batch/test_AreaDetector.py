...
class Test_AreaDetector:
    ...
    def test_detect_connected_regions(self):
        ...
        partition, num_regions = area_detector._detect_connected_regions(fcondition, edgeface_indexes)
    ...
    def test_comp_sedimentation_volume2(self):
        ...
        area_detector = AreaDetector()
        dvol_result, area_eq_result, dvol_eq_result = area_detector._comp_sedimentation_volume2(dzgem, dzmin, area, slength, nwidth)

        expected_dvol = 20.833333333333336
        expected_area_eq = 1200.0
        expected_dvol_eq = 500.0

        assert numpy.isclose(dvol_result, expected_dvol), f"Incorrect dvol: {dvol_result}"
        assert numpy.isclose(area_eq_result, expected_area_eq), f"Incorrect area_eq: {area_eq_result}"
        assert numpy.isclose(dvol_eq_result, expected_dvol_eq), f"Incorrect dvol_eq: {dvol_eq_result}"
    ...
    def test_comp_sedimentation_volume1_tot(self):
        ...
        area_detector = AreaDetector()
        tot_dredge_vol, wght_all_dredge_sed = area_detector._comp_sedimentation_volume1_tot(sedvol, sbin, afrac, siface, sbin_length, slength )
        ...
        assert numpy.isclose(wght_all_dredge_sed.all(), expected_wght_all_dredge_sed.all()), f"Incorrect wght_all_dredge_sed: {wght_all_dredge_sed}"
    ...
    def test_comp_sedimentation_volume1_one_width_bin(self):
        ...
        with (patch("dfastmi.batch.AreaDetector.AreaDetector._comp_sedimentation_volume1_tot") as mock_comp_sedimentation_volume1_tot):
            mock_comp_sedimentation_volume1_tot.return_value = (expected_tot_dredge_vol, expected_wght_all_dredge)

            area_detector = AreaDetector()
            tot_dredge_vol, wght_all_dredge = area_detector._comp_sedimentation_volume1_one_width_bin(dvol, sbin, afrac, siface, sbin_length, slength)
