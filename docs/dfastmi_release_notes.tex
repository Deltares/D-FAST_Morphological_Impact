...
\endfoot
\endlastfoot
DFAST-313 & Bug & Morphological effective period should depend on the stagnant discharge instead of threshold discharge (since 3.1.3) \\
DFAST-299 & Bug & \dfmi results file contained invalid \texttt{face\_node\_connectivity} data when using Fourier files as data source (since 3.1.3) \\
DFAST-298 & Bug & Classic WAQMORF mode failed for \dflowfm NetCDF simulation files when low flow condition was not needed (since 3.1.3) \\
DFAST-213 & Bug & The threshold discharge in the GUI was not always set to the value specified in the loaded config-file (since 3.1.3) \\
DFAST-279 & Bug & \dfmi corrects for differences in mesh topology between reference and intervention runs (since 3.1.2) \\
DFAST-241 & Doc & Add \dfmi validation document (since patch 3.1.1) \\
