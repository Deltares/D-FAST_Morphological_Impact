...
class DFastMIConfigParser:
    ...
    def __init__(self, config: configparser.ConfigParser):
        ...
        self._processor.register_processor(
            bool, self._process_entry_value, self._parse_bool
        )
        self._processor.register_processor(
            int, self._process_entry_value, self._parse_int
        )
        self._processor.register_processor(
            float, self._process_entry_value, self._parse_float
        )
        self._processor.register_processor(
            Tuple[float, ...], self._process_tuple_entry_value, self._parse_float
        )
        self._processor.register_processor(
            Tuple[str, ...], self._process_tuple_entry_value, self._parse_str
        )
    ...
    # default processor
    def _process_entry_value(
        self,
        key,
        entry_value: str,
        reach: AReach,
        parse: Callable[[str], Tuple[T, ...]],
        default: Optional[T],
        expected_number_of_values: Optional[int],
    ) -> T:
    ...
    def _parse_bool(self, entry_value) -> Tuple[bool, ...]:
        return tuple(
            x.lower() in ["true", "1", "t", "y", "yes"] for x in entry_value.split()
        )
    ...
    # tuple processor
    def _process_tuple_entry_value(
        self,
        key,
        entry_value: str,
        reach: AReach,
        parse: Callable[[str], Tuple[T, ...]],
        default: Optional[Tuple[T, ...]],
        expected_number_of_values: Optional[int],
    ) -> Tuple[T, ...]:
        vals: Tuple[T, ...]
        if entry_value == "" and default is not None:
    ...
    def read_key(
        ...
        data : int
            ...
            branch one float.
        """
        entry_value = self._read_value(
            key, reach.parent_branch.name, reach.config_key_index
        )
        return self._processor.process_river_element(
            value_type, key, entry_value, reach, default, expected_number_of_values
        )
    ...
    def config_get(
        ...
        try:
            ...
            val = self._config_processor.process_config_value(value_type, entry_value)[
                0
            ]
        except:
            if default is not None:
                ...
                raise Exception(
                    f'No {value_type.__name__} value specified for required keyword "{key}" in block "{group}".'
                )
