...
\item The generated figure should correspond to \autoref{fig:report_GendtseWaard.png}.
...
\begin{figure}[H]
	...
	\caption{QUICKPLOT settings to create \autoref{fig:report_GendtseWaard_QP.png}.}
	\label{fig:manual_system_test_8_quickplot_usage.png}
\end{figure}

\begin{figure}[H]
	\center
	\includegraphics[width=12cm]{figures/GendtseWaard_jaargem.png}
	\caption{Analysis results as plotted using QUICKPLOT.}
	\label{fig:report_GendtseWaard_QP.png}
\end{figure}
...
-----------------------------------------------------------------------
dfastmi\__init__.py                                     8      2    75%
dfastmi\__main__.py                                    65     65     0%
...
dfastmi\batch\AnalyserDflowfm.py                      237     12    95%
dfastmi\batch\AnalyserWaqua.py                        150      0   100%
dfastmi\batch\AreaDetector.py                         111      0   100%
dfastmi\batch\AreaPlotter.py                           78      0   100%
dfastmi\batch\DFastUtils.py                            49     38    22%
dfastmi\batch\DflowfmReporters.py                      95     33    65%
dfastmi\batch\Distance.py                              54      4    93%
...
dfastmi\batch\Projection.py                            47     10    79%
...
dfastmi\batch\core.py                                 224     15    93%
dfastmi\batch\plotting.py                             121     50    59%
dfastmi\cli.py                                        237     53    78%
...
dfastmi\config\ConfigFileOperations.py                131     13    90%
dfastmi\config\ConfigurationChecker.py                 59     11    81%
dfastmi\config\ConfigurationCheckerFactory.py          22      0   100%
dfastmi\config\ConfigurationCheckerLegacy.py           55      4    93%
dfastmi\config\ConfigurationCheckerValidator.py        14      0   100%
dfastmi\config\ConfigurationInitializer.py            104      5    95%
dfastmi\config\ConfigurationInitializerFactory.py      25      1    96%
dfastmi\config\ConfigurationInitializerLegacy.py       52      0   100%
dfastmi\gui\dialog_model.py                           131      2    98%
dfastmi\gui\dialog_utils.py                            87     25    71%
dfastmi\gui\dialog_view.py                            451     34    92%
dfastmi\gui\dialog_view_model.py                      208      6    97%
dfastmi\gui\qt_tools.py                                20      6    70%
dfastmi\io\AReach.py                                   27      1    96%
dfastmi\io\ApplicationSettingsHelper.py                47      4    91%
dfastmi\io\Branch.py                                   40      2    95%
dfastmi\io\CelerObject.py                              54      1    98%
...
dfastmi\io\DataTextFileOperations.py                   37      1    97%
dfastmi\io\FouFile.py                                  16      0   100%
dfastmi\io\IBranch.py                                  17      0   100%
dfastmi\io\IReach.py                                   15      0   100%
dfastmi\io\MapFile.py                                  73      0   100%
dfastmi\io\ObservableList.py                           26      0   100%
dfastmi\io\OutputFile.py                               80      0   100%
dfastmi\io\OutputFileFactory.py                        26      0   100%
dfastmi\io\Reach.py                                    49      0   100%
dfastmi\io\ReachLegacy.py                              12      0   100%
dfastmi\io\RiversObject.py                            121      2    98%
dfastmi\kernel\BedLevelCalculator.py                   60      0   100%
dfastmi\kernel\core.py                                 30      0   100%
dfastmi\kernel\legacy.py                               55      0   100%
dfastmi\kernel\typehints.py                             9      0   100%
dfastmi\resources.py                                    4      0   100%
-----------------------------------------------------------------------
TOTAL                                                4619    441    90%

====================== 487 passed, 34 warnings in 53.05s ======================
\end{Verbatim}
...
SonarCloud reports a major jump in the code duplication and issues in May 2025.
The code wasn't changed during that period; it is due to an update to the SonarCloud analysis ... as is obvious from the large jump in the number of lines of code at the same time.
Before that time, SonarCloud only analysed the code lines of the \dfmi program; now it also analyses the lines of testing code.
That is where the code duplication is located (see the listed files in \autoref{fig:code_duplication}, and the remark at the bottom that there are 115 files with 0\% duplication).
This is also where 90\% of the reported code issues are; see \autoref{fig:code_issues}.

\begin{figure}
\center
\includegraphics[width=\textwidth]{figures/code_duplication.png}
\includegraphics[width=\textwidth]{figures/code_duplication_details.png}
\caption{Code duplication reported by SonarCloud}
...
Sigrid scores the overall maintainability at 3.8, with an architecture score of 3.2.
See Sigrid overview report in \autoref{fig:sigrid_overview}.
The total amount of code represents 13 person months (to develop).
The number of lines of code for testing is almost the same as the number of lines of code for the program itself (test/code ration = 97 \%).
The open source health is scored at 3.0: the code depends on 68 open source libraries of which 7 (cftime, intel-openmp, mkl, tbb, pandas, pyqt5. pyqt5-qt5) are marked as medium to high risk since the version used is more than 1 year old.
Three of the libraries (intel-openmp, mkl, tbb) are also flagged for proprietary licenses; for 30 libraries Sigrid doesn't know the license conditions.
The are no security or reliability issues.
\Autoref{fig:sigrid_architecture_overview} shows the Sigrid architecture overview.
In particular the "component freshness" score has decreased because changes have been limited to one component instead of improving all components; this is natural for a stable code base with minor fixes.
The "communication centralization" scores now significantly better than before and the "bounded evolution" is no longer evaluated.
\Autoref{fig:sigrid_architecture_comm} shows the Sigrid communication centralization overview.
The Sigrid score for this topic is based on the number of calls between components compared to the number of calls within a component; nothing has changed here, but the score has improved ... this may be due to code stability which causes the interfaces to remain stable.
The "bounded evolution" score determines how often files are changed in the same commit.
Because there have been very few code changes, there wasn't enough data to check for correlated changes across multiple components.
