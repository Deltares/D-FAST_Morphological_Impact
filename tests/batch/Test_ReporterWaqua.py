...
class Test_ReporterWaqua:
    def given_output_data_and_mocked_write_files_when_write_report_then_expect_3_calls_for_writing_report(
        self,
    ):
        """
        ...
        with (
            patch(
                "dfastmi.batch.ReporterWaqua.ApplicationSettingsHelper.get_filename"
            ) as mocked_get_file_name,
            patch(
                "dfastmi.batch.ReporterWaqua.DataTextFileOperations.write_simona_box"
            ) as mocked_write_simona_box,
        ):
