...
The version number should be \keyw{3}.\footnote{Initially this file version was named \keyw{2} but to avoid confusion with the fact that this file format is only supported by \dfastmi version 3.0.0, the version number has been increased.
Files with version \keyw{2} are still accepted, but not recommended.}

Besides the \keyw{[General]} block version 3 files should only contain data blocks identifying the river branches (in Dutch: takken).
...
\endfoot
\endlastfoot
\keyw{General} & \keyw{Version} & Version number of the file format.
Should be \keyw{3} or equivalent e.g.\ \keyw{3.0} or \keyw{3.0.0}. \\
\keyw{General} & \keyw{ConfigVersion} & Version number for the actual configuration stored in the file.
This keyword is intended for version management of the file content.
It is currently not used by the software. \\
...
Version 3 files must contain in the \keyw{[General]} block also the keywords \keyw{Branch} and \keyw{Reach} to identify the branch (in Dutch: tak) and reach (in Dutch: stuk) in which the intervention is located.
