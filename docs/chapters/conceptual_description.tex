...
\item[$s_\text{mc}$] main channel unit sediment transport \unitbrackets{m\textsuperscript{2}/s}
\item[$s_\text{out}$] \emph{outflow of} unit sediment \emph{to} the flood plan per unit length \unitbrackets{m\textsuperscript{2}/ms}
\end{symbollist}
...
The lower bound for the morphological impact of the measure will be equal to the minimum value of $z_{b,1}(0)$, $z_{b,2}(0)$ and $z_{b,3}(0)$ and the upper bound is given by the maximum value of $z_{b,1}(0)$, $z_{b,2}(0)$, $z_{b,3}(0)$.
For the schematized hydrograph shown in \autoref{Fig5} the bed level change is

\begin{itemize}
\item at its maximum after flood period 3 (the third block in \autoref{Tab2}), the corresponding bed level is $z_{b,1}(0)$ (\autoref{Eq8e}) 
\item at its minimum after the low flow period 1 (the first block in \autoref{Tab2}), the corresponding bed level is $z_{b,2}(0)$ (\autoref{Eq8f}).
\end{itemize}
...
After identifying expressions for the two extreme (minimum, maximum) values, we also determine an expression for the year-averaged bed level change by integrating \autoref{Eq7} per constant discharge period to
%
\begin{align}
\bar{z}_{b,i} &= \frac{1}{T_i} \int_0^{T_i}{z_{b,i} (0) + [z_{b,i,\text{eq}} - z_{b,i}(0)](1 - e^{-t/T_{m,i}})}dt \nonumber \\
&= \frac{1}{T_i} \int_0^{T_i}{z_{b,i,\text{eq}} + [z_{b,i}(0) - z_{b,i,\text{eq}}]e^{-t/T_{m,i}}}dt \nonumber \\
&= \frac{1}{T_i} \left . \left ( {z_{b,i,\text{eq}} t - [z_{b,i}(0) - z_{b,i,\text{eq}}]T_{m,i} e^{-t/T_{m,i}}} \right ) \right |_{t=0}^{t=T_i} \nonumber \\
&= z_{b,i,\text{eq}} - [z_{b,i}(0) - z_{b,i,\text{eq}}] \frac{T_{m,i}}{T_i} ( e^{-T_i/T_{m,i}} - 1 ) \nonumber \\
&= z_{b,i,\text{eq}} + [z_{b,i}(0) - z_{b,i,\text{eq}}] \frac{T_{m,i}}{T_i} ( 1 - \sigma_i ) \label{Eq8h_1}
\end{align}
%
One may notice that for a large morphological time scale $T_{m,i}$, the last term $\sigma_i - 1$ will become very small while the $T_{m,i}$ term becomes obviously large; their product will tend to $T_i$.
Alternatively, for $T_i \ll T_{m,i}$ one can assume the transition from $z_{b,i}(0)$ to $z_{b,i+1}(0)$ to be approximately linear, which results in a much simpler expression
%
\begin{equation}
\bar{z}_{b,i} \approx \frac{1}{2} [ z_{b,i+1}(0) + z_{b,i}(0) ] \label{Eq8hv2_1}
\end{equation}
%
The integration over all periods results in
%
\begin{equation}
\bar{z}_b = \frac{\sum{z_{b,i,\text{eq}} T_i}}{\sum{T_i}} + \frac{\sum{[ z_{b,i}(0) - z_{b,i,\text{eq}} ] T_{m,i} ( 1 - \sigma_i )}}{\sum{T_i}}
\label{Eq8h}
\end{equation}
%
based on \autoref{Eq8h_1} and based on \autoref{Eq8hv2_1} we obtain
%
\begin{equation}
\bar{z}_b \approx \frac{\sum [ z_{b,i+1}(0) + z_{b,i}(0) ] T_i}{2 \sum{T_i}}
\label{Eq8hv2}
\end{equation}
%
\dfastmi uses \autoref{Eq8hv2} to compute the mean bed level change.
