...
The response of the main channel is represented here by a bed level change $\Delta z$ \unitbrackets{m} (raise or lowering) which is small relative to the local water depth.
...
\pdiff{q_\text{mc}}{s} = -q_\text{out}
\end{equation}} \label{Eq2} \\
sediment & \parbox{\textwidth-4cm}{\begin{equation}
\pdiff{z_b}{t} + \pdiff{s_\text{mc}}{s}= -q_\text{out} = -s_\text{out}
...
\begin{symbollist}
\item[$q_\text{mc}$] \unitbrackets{m\textsuperscript{2}/s} main channel unit discharge
\item[$q_\text{out}$] \unitbrackets{m\textsuperscript{2}/sm} \emph{outflow of} unit discharge \emph{to} the flood plain per unit length
\item[$s$] \unitbrackets{m} streamwise coordinate
\item[$z_b$] \unitbrackets{m} bed level change due to measure
\item[$s_\text{mc}$] \unitbrackets{m\textsuperscript{2}/s} main channel unit sediment transport
\item[$s_\text{out}$] \unitbrackets{m\textsuperscript{2}/ms} \emph{outflow of} unit sediment \emph{to} the flood plan per unit length
\end{symbollist}

The unit sediment transport capacity in the main channel, $s_\text{mc}$, is approximated by $s_\text{mc} = m \left ( \frac{q_\text{mc}}{h} \right )^b$ with $h$ the water depth \unitbrackets{m} in the main channel, $m$ a calibration factor and sediment transport exponent $b$ about 5 \citep{Engelundh67}.
Using this approximation the gradients in the sediment transport capacity can be written as

\begin{equation}
\pdiff{s_\text{mc}}{s} \approx b \frac{s_\text{mc}}{q_\text{mc}} \pdiff{q_\text{mc}}{s} + b \frac{s_\text{mc}}{h} \pdiff{z_b}{s}
\label{Eq4}
\end{equation}

Substitution of \autoref{Eq4} in \autoref{Eq2} and \autoref{Eq3}, the mass balances for water and sediment, gives
...
\begin{equation}
\diff{\Delta z_b}{t} = \frac{\Delta z_{b,\text{eq}} - \Delta z}{T}
\end{equation}

with a morphological time scale

\begin{equation}
T \approx \frac{h L}{b s_\text{mc}}
\label{Eq5T}
\end{equation}
...
The equilibrium value $\Delta z_{\text{eq}}$ depends according \autoref{Eq6} on the (original) water depth, the relative change in unit discharge and the relative change in the sediment transport.
...
The bed level development during a relaxation period can (as a solution of \autoref{Eq6}) is given by
...
We can't only obtain expressions for the two extreme (minimum, maximum) values but we can also determine an expression for the year-averaged bed level change by integrating \autoref{Eq7} to

\begin{equation}
\bar{z_b} = \frac{\sum{z_{b,i,\text{eq}} t_i}}{\sum{t_i}} + \frac{\sum{(z_{b,i,\text{eq}}-z_{b,i}(0)) T_{m,i} (\sigma_i-1)}}{\sum{t_i}}
\label{Eq8h}
\end{equation}
