...
In 2022, the pilot implementation of the \emph{stepped-hydrograph} approach was tested using two cases for which the \dfastmi results (using steady state flow fields obtained using Delft3D-FLOW) were compared against long-term morphological simulations carried out using Delft3D-FLOW.\footnote{Delft3D-FLOW is the combined flow and morphology component of the Delft3D 4 Suite.}
...
Since the DVR model was too coarse to represent the actual secondary channel, a combination of flow extraction (at Rkm 910.5) and insertion (at Rkm 912) was used to represent the side channel, see \autoref{Palmers_proj}.
...
\caption{Side channel design (left; created using ArcGIS) and initial representation on the grid (right).}
\label{Palmers_proj}
\end{figure}
...
\autoref{Palmers_mor} shows the results of a long-term (12 year) reference morphological simulation using Delft3D-FLOW (for details, see \citet{GiriJagers2022}) and the year-averaged equilibrium morphological impact as computed by \dfmi version 3.
The impacted length is estimated to be \SI{721}{\metre}.
The results match well although the asymmetry downstream of the main sedimentation patch differs.
The along-steam (1D) variability is general reproduced better than the 2D spatial patterns since the conceptual basis of \dfmi is a 1D-approach in which lateral redistribution of the flow due to morphological change is ignored.
This is due to the lack of flow-morphology interaction in the simplified \dfmi analysis.
Using the 6 selected steady discharges is sufficient for this analysis.
...
Since the grid was too coarse to represent the actual secondary channel, a combination of flow extraction and insertion was used to represent the side channel, see \autoref{Pannerden_proj}.
...
\autoref{Pannerden_mor} shows the results of a long-term (15 year) reference morphological simulation using Delft3D-FLOW (for details, see \citet{GiriJagers2022}) and the year-averaged equilibrium morphological impact as computed by \dfmi version 3.
The impacted length is estimated to be \SI{1084}{\metre}.
...
The results are judged as valid and sufficient for the purpose of \dfmi.